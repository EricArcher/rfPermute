...
#' @details The function will generate an panel of plots, one for each importance type.
...
plot.rp.importance <- function(x, alpha = 0.05, sig.only = FALSE, n = NULL, main = NULL, ...) {
  ...
  stopifnot(require(gridExtra, quietly = TRUE))
  ...
  imp.list <- lapply(seq(1, ncol(x), 2), function(i) {
    ...
    if(sig.only) imp.df <- subset(imp.df, as.logical(is.sig))
    if(!is.null(n) & is.numeric(n)) imp.df <- imp.df[1:min(n, nrow(imp.df)), ]
    ggplot(imp.df, aes(imp, reorder(pred, imp))) + 
